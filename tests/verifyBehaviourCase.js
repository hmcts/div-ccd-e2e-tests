...
Scenario('verify all tab fields of PFE, RFE, DN, DA', async function (I) {
  ...
  await I.validateAOSTabData(reasonsForDivorce.BEHAVIOUR, verifyContent);
  await I.validateDecreeNisiTabData(reasonsForDivorce.BEHAVIOUR, verifyContent);
  await I.validateOutcomeOfDNTabData(verifyContent);
  await I.validateDecreeAbsoluteTabData(verifyContent);
  await I.validateDocumentTabData(reasonsForDivorce.BEHAVIOUR, caseId);
